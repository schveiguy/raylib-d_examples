...
void main()
{
    ...
    auto file1 = (thisExePath().dirName ~ "/../resources/texel_checker.png").buildNormalizedPath.toStringz;
    Texture texture = LoadTexture(file1);
    ...
    auto fs = (thisExePath().dirName ~ "/../resources/shaders/" ~
                TextFormat("glsl%i/fog.fs", GLSL_VERSION).to!string).buildNormalizedPath.toStringz;
    ...
    CreateLight(LIGHT_POINT, Vector3( 0, 2, 6 ), Vector3Zero(), Colors.WHITE, shader);
    ...
    while (!WindowShouldClose())        // Detect window close button or ESC key
    {
        ...
        SetShaderValue(shader, shader.locs[ShaderLocationIndex.SHADER_LOC_VECTOR_VIEW], &camera.position.x, ShaderUniformDataType.SHADER_UNIFORM_VEC3);
        //----------------------------------------------------------------------------------
        ...
        BeginDrawing();
            ...
            BeginMode3D(camera);
                ...
                DrawModel(modelA, Vector3Zero(), 1.0f, Colors.WHITE);
                DrawModel(modelB, Vector3( -2.6f, 0, 0 ), 1.0f, Colors.WHITE);
                DrawModel(modelC, Vector3( 2.6f, 0, 0 ), 1.0f, Colors.WHITE);

                for (int i = -20; i < 20; i += 2) {
                    DrawModel(modelA, Vector3( cast(float)i, 0, 2 ), 1.0f, Colors.WHITE);
                }

            EndMode3D();

            DrawText(TextFormat("Use KEY_UP/KEY_DOWN to change fog density [%.2f]", fogDensity), 10, 10, 20, Colors.RAYWHITE);
