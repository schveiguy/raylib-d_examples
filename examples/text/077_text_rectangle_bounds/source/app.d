...
void main()
{
    ...
    Color borderColor = Colors.MAROON;         // Container border color
    ...
    while (!WindowShouldClose())        // Detect window close button or ESC key
    {
        // Update
        //----------------------------------------------------------------------------------
        if (IsKeyPressed(KeyboardKey.KEY_SPACE)) wordWrap = !wordWrap;

        Vector2 mouse = GetMousePosition();

        // Check if the mouse is inside the container and toggle border color
        if (CheckCollisionPointRec(mouse, container)) {
            borderColor = Fade(Colors.MAROON, 0.4f);
        }
        else if (!resizing) {
        ...
        BeginDrawing();
            ...
            DrawTextBoxed(font, text.toStringz, Rectangle( container.x + 4, container.y + 4, container.width - 4, container.height - 4 ), 20.0f, 2.0f, wordWrap, Colors.GRAY);

            DrawRectangleRec(resizer, borderColor);             // Draw the resize box

            // Draw bottom info
            DrawRectangle(0, screenHeight - 54, screenWidth, 54, Colors.GRAY);
            DrawRectangleRec(Rectangle( 382.0f, screenHeight - 34.0f, 12.0f, 12.0f ), Colors.MAROON);

            DrawText("Word Wrap: ", 313, screenHeight-115, 20, Colors.BLACK);
            if (wordWrap) DrawText("ON", 447, screenHeight - 115, 20, Colors.RED);
            else DrawText("OFF", 447, screenHeight - 115, 20, Colors.BLACK);

            DrawText("Press [SPACE] to toggle word wrap", 218, screenHeight - 86, 20, Colors.GRAY);

            DrawText("Click hold & drag the    to resize the container", 155, screenHeight - 38, 20, Colors.RAYWHITE);
...
static void DrawTextBoxedSelectable(Font font, const char *text, Rectangle rec, float fontSize, float spacing, bool wordWrap, Color tint, int selectStart, int selectLength, Color selectTint, Color selectBackTint)
{
    ...
    const int MEASURE_STATE = 0;
    const int DRAW_STATE = 1;
