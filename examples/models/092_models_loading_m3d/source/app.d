...
void main()
{
    ...
    auto modelFileName = (thisExePath().dirName ~ "/../resources/models/m3d/cesium_man.m3d").buildNormalizedPath.toStringz;
    ...
    while (!WindowShouldClose())        // Detect window close button or ESC key
    {
        // Update
        //----------------------------------------------------------------------------------
        UpdateCamera(&camera, CameraMode.CAMERA_ORBITAL); // CAMERA_FIRST_PERSON

        if (animsCount)
        {
            // Play animation when spacebar is held down (or step one frame with N)
            if (IsKeyDown(KeyboardKey.KEY_SPACE) || IsKeyPressed(KeyboardKey.KEY_N))
            {
                animFrameCounter++;
        ...
        BeginDrawing();
            ...
            BeginMode3D(camera);
                ...
                if (drawMesh) {
                    DrawModel(model, position, 1.0f, Colors.WHITE);
                }
                ...
                if (drawSkeleton)
                {
                    ...
                    for (int i = 0; i < model.boneCount - 1; i++)
                    {
                        ...
                        if (!animPlaying || !animsCount)
                        {
                            ...
                            DrawCube(model.bindPose[i].translation, 0.04f, 0.04f, 0.04f, Colors.RED);

                            if (model.bones[i].parent >= 0)
                            {
                                DrawLine3D(model.bindPose[i].translation,
                                    model.bindPose[model.bones[i].parent].translation, Colors.RED);
                            }
                        }
                        else
                        {
                            // Display the frame-pose skeleton
                            DrawCube(anims[animId].framePoses[animFrameCounter][i].translation, 0.05f, 0.05f, 0.05f, Colors.RED);

                            if (anims[animId].bones[i].parent >= 0)
                            {
                                DrawLine3D(anims[animId].framePoses[animFrameCounter][i].translation,
                                    anims[animId].framePoses[animFrameCounter][anims[animId].bones[i].parent].translation, Colors.RED);
                            }
                        }
                    }
                }
            ...
            DrawText("PRESS SPACE to PLAY MODEL ANIMATION", 10, GetScreenHeight() - 60, 10, Colors.MAROON);
            DrawText("PRESS A to CYCLE THROUGH ANIMATIONS", 10, GetScreenHeight() - 40, 10, Colors.DARKGRAY);
            DrawText("PRESS M to toggle MESH, S to toggle SKELETON DRAWING", 10, GetScreenHeight() - 20, 10, Colors.DARKGRAY);
            DrawText("(c) CesiumMan model by KhronosGroup", GetScreenWidth() - 210, GetScreenHeight() - 20, 10, Colors.GRAY);
