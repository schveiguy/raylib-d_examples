...
import core.stdc.stdlib;                 // Required for: calloc(), free()

const int MAP_TILE_SIZE          = 32;   // Tiles size 32x32 pixels
const int PLAYER_SIZE            = 16;   // Player size
const int PLAYER_TILE_VISIBILITY =  2;   // Player can see 2 tiles around its position

// Map data type
struct Map {
    uint tilesX;            // Number of tiles in X axis
    uint tilesY;            // Number of tiles in Y axis
    ubyte *tileIds;         // Tile ids (tilesX*tilesY), defines type of tile to draw
    ubyte *tileFog;         // Tile fog state (tilesX*tilesY), defines if a tile has fog or half-fog
}
...
void main()
{
    ...
    map.tileIds = cast(ubyte*)calloc(map.tilesX*map.tilesY, ubyte.sizeof);
    map.tileFog = cast(ubyte*)calloc(map.tilesX*map.tilesY, ubyte.sizeof);

    // Load map tiles (generating 2 random tile ids for testing)
    // NOTE: Map tile ids should be probably loaded from an external map file
    for (uint i = 0; i < map.tilesY*map.tilesX; i++) {
        map.tileIds[i] = cast(ubyte)GetRandomValue(0, 1);
    }
    ...
    SetTextureFilter(fogOfWar.texture, TextureFilter.TEXTURE_FILTER_BILINEAR);
    ...
    while (!WindowShouldClose())    // Detect window close button or ESC key
    {
        ...
        //----------------------------------------------------------------------------------
        // Move player around
        if (IsKeyDown(KeyboardKey.KEY_RIGHT)) playerPosition.x += 5;
        if (IsKeyDown(KeyboardKey.KEY_LEFT))  playerPosition.x -= 5;
        if (IsKeyDown(KeyboardKey.KEY_DOWN))  playerPosition.y += 5;
        if (IsKeyDown(KeyboardKey.KEY_UP))    playerPosition.y -= 5;
        ...
        if (playerPosition.x < 0) {
            ...
            playerPosition.x = cast(float)map.tilesX*MAP_TILE_SIZE - PLAYER_SIZE;
        }
        if (playerPosition.y < 0) {
            playerPosition.y = 0;
        }
        else if ((playerPosition.y + PLAYER_SIZE) > (map.tilesY*MAP_TILE_SIZE)) {
            playerPosition.y = cast(float)map.tilesY*MAP_TILE_SIZE - PLAYER_SIZE;
        }
        ...
        playerTileX = cast(int)((playerPosition.x + MAP_TILE_SIZE/2)/MAP_TILE_SIZE);
        playerTileY = cast(int)((playerPosition.y + MAP_TILE_SIZE/2)/MAP_TILE_SIZE);
        ...
        for (int y = (playerTileY - PLAYER_TILE_VISIBILITY); y < (playerTileY + PLAYER_TILE_VISIBILITY); y++) {
            for (int x = (playerTileX - PLAYER_TILE_VISIBILITY); x < (playerTileX + PLAYER_TILE_VISIBILITY); x++) {
                if ((x >= 0) && (x < cast(int)map.tilesX) && (y >= 0) && (y < cast(int)map.tilesY)) {
            ...
            for (uint y = 0; y < map.tilesY; y++) {
                for (uint x = 0; x < map.tilesX; x++) {
                    if (map.tileFog[y*map.tilesX + x] == 0) {
                        DrawRectangle(x, y, 1, 1, Colors.BLACK);
                    }
                    else if (map.tileFog[y*map.tilesX + x] == 2) {
                        DrawRectangle(x, y, 1, 1, Fade(Colors.BLACK, 0.8f));
                    }
                }
            }
        EndTextureMode();
        ...
        BeginDrawing();
            ...
            for (uint y = 0; y < map.tilesY; y++)
            {
                for (uint x = 0; x < map.tilesX; x++)
                {
                    ...
                    DrawRectangle(x*MAP_TILE_SIZE, y*MAP_TILE_SIZE, MAP_TILE_SIZE, MAP_TILE_SIZE,
                                  (map.tileIds[y*map.tilesX + x] == 0) ? Colors.BLUE : Fade(Colors.BLUE, 0.9f));
                    DrawRectangleLines(x*MAP_TILE_SIZE, y*MAP_TILE_SIZE, MAP_TILE_SIZE, MAP_TILE_SIZE, Fade(Colors.DARKBLUE, 0.5f));
                }
            }

            // Draw player
            DrawRectangleV(playerPosition, Vector2( PLAYER_SIZE, PLAYER_SIZE ), Colors.RED);
            ...
            DrawText(TextFormat("Current tile: [%i,%i]", playerTileX, playerTileY), 10, 10, 20, Colors.LIME);
